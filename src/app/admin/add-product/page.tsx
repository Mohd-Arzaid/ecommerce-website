...
const AddProductPage = () => {
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    // User ne jo images select ki hai, unhe array m convert kardo
    const files = Array.from(e.target.files || []);

    // agar no image select , then function will return nothing
    if (!files.length) return;

    // Old images + new images ko combine kardo
    // slice(0, 4) max 4 images hi rakhege
    setImages((prev) => [...prev, ...files].slice(0, 4));

    // Input ko reset kardo
    // Taki same image ko dobara select karne par bhi event trigger ho sake
    e.target.value = "";
  };

  const removeImage = (index: number) => {
    // example : Prev m 4 images hai, index wali image ko hata do
    // [img1, img2, img3, img4] → index 1 remove → [img1, img3, img4]
